...
function isAllowedBaseUrl(baseUrl: string, options?: SsrfOptions): boolean;
```
...
| `fe80::/10` (regex, bracket-aware) | IPv6 link-local (RFC 4291) |
...
- `ipc-validation.ts` — imports `isAllowedBaseUrl` for IPC boundary checks (allowLoopback: true)
- `index.ts` — imports `isHostAllowed`, `isUrlAllowedForFetch`, `isAllowedBaseUrl` for API request/stream checks

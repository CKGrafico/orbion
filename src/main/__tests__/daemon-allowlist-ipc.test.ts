...
describe("api:request — allowlist enforcement", () => {
  ...
  it("allows GET /api/loops (allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops", method: "GET" }]),
    ).not.toThrow();
  });

  it("allows POST /api/loops/{id}/pause (allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123/pause", method: "POST" }]),
    ).not.toThrow();
  });

  it("allows GET /api/loops/{id}/logs?tail=50 (allowlisted with query)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123/logs?tail=50", method: "GET" }]),
    ).not.toThrow();
  });

  it("rejects DELETE /api/loops/{id} (not allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123", method: "DELETE" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects POST /api/settings (not allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/settings", method: "POST" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects PATCH /api/loops/{id} (not allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123", method: "PATCH" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects POST /api/loops (not allowlisted — no create-from-renderer)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops", method: "POST" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects GET /api/admin (not allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/admin", method: "GET" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects DELETE /api/loops (not allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops", method: "DELETE" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects POST /api/loops/{id}/execute (arbitrary command path)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123/execute", method: "POST" }]),
    ).toThrow(IpcValidationError);
  });
  ...
  it("defaults method to GET when omitted (and still enforces allowlist)", () => {
    ...
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops" }]),
    ).not.toThrow();

    // GET /api/admin is NOT allowlisted
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/admin" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects DELETE /api/projects (mutation on wrong endpoint)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/projects", method: "DELETE" }]),
    ).toThrow(IpcValidationError);
  });
  ...
  it("rejects POST /api/users (arbitrary entity creation)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/users", method: "POST" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects DELETE /api/loops/{id}/runs/{runId} (arbitrary nested path)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.API_REQUEST, [{ baseUrl: validBaseUrl, path: "/api/loops/abc-123/runs/run-42", method: "DELETE" }]),
    ).toThrow(IpcValidationError);
  });
});
...
describe("stream:subscribe — allowlist enforcement", () => {
  ...
  it("allows /api/loops/{id}/logs/stream (allowlisted)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-1", baseUrl: validBaseUrl, path: "/api/loops/abc-123/logs/stream" }]),
    ).not.toThrow();
  });

  it("allows /api/loops/{id}/logs/stream with query string", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-2", baseUrl: validBaseUrl, path: "/api/loops/abc-123/logs/stream?tail=0" }]),
    ).not.toThrow();
  });

  it("rejects /api/loops/{id}/logs (non-stream endpoint)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-3", baseUrl: validBaseUrl, path: "/api/loops/abc-123/logs" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects /api/events (arbitrary SSE endpoint)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-4", baseUrl: validBaseUrl, path: "/api/events" }]),
    ).toThrow(IpcValidationError);
  });

  it("rejects /api/admin/stream (arbitrary stream path)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-5", baseUrl: validBaseUrl, path: "/api/admin/stream" }]),
    ).toThrow(IpcValidationError);
  });

  it("includes allowlist message in error for disallowed stream paths", () => {
    try {
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-6", baseUrl: validBaseUrl, path: "/api/events" }]);
      expect.unreachable("Should have thrown");
    } catch (err) {
      expect(err).toBeInstanceOf(IpcValidationError);
      const msg = (err as IpcValidationError).message;
  ...
  it("rejects /api/loops/{id}/exec/stream (arbitrary command stream)", () => {
    expect(() =>
      validateIpc(IPC_CHANNELS.STREAM_SUBSCRIBE, [{ subId: "sub-7", baseUrl: validBaseUrl, path: "/api/loops/abc-123/exec/stream" }]),
    ).toThrow(IpcValidationError);
  });
});

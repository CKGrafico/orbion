...
function deriveItems(params: InboxBuildParams): InboxItem[] {
  const { perEnvLoops, perEnvHealth, environments, breaches, dismissedIds, escalatedOutages, trippedWatches } = params;
...
function answerFleetQuery(
  ...
  // "watches" / "watch" / "notifications" / "alerts" / "tripped" / "ping"
  const isWatchQuery =
    q.includes("watch") ||
    q.includes("tripped") ||
    q.includes("alert") ||
